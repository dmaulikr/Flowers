...
@implementation FifthLevels
+(id) scene {
	// 'scene' is an autorelease object.
	CCScene *scene = [CCScene node];
	
	// 'layer' is an autorelease object.
	FifthLevels *layer = [FifthLevels node];
...
-(void)updateTime:(id)sender {
	...
	if(changingLevel) {
			[SingletonGameState sharedGameStateInstance].time += 15;
			changingLevel = FALSE;
		}
...
//	NSLog(@"Current level: %i", [SingletonGameState sharedGameStateInstance].level);
	...
	if( (self=[super init] )) {
		...
		Flower* goodFlower = [Flower spriteWithFile: @"badFlower3.png"];
...
//				NSLog(@"Good flower touched.");
				if([SingletonGameState sharedGameStateInstance].level >= 25) {
					[[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:1 scene:[YouWin node]]];	
				}
				else {
