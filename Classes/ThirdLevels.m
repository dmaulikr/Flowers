...
@implementation ThirdLevels
+(id) scene {
	// 'scene' is an autorelease object.
	CCScene *scene = [CCScene node];
	
	// 'layer' is an autorelease object.
	ThirdLevels *layer = [ThirdLevels node];
...
-(id) init
{
//	NSLog(@"Trying to load third set of levels.");
...
//	NSLog(@"Current level: %i", [SingletonGameState sharedGameStateInstance].level);
	...
	if( (self=[super init] )) {
		...
		Flower* goodFlower = [Flower spriteWithFile: @"flower4.png"];
...
//				NSLog(@"Good flower touched.");
				if([SingletonGameState sharedGameStateInstance].level >= 15) {
