...
@implementation FirstLevels
+(id) scene {
	// 'scene' is an autorelease object.
	CCScene *scene = [CCScene node];
	
	// 'layer' is an autorelease object.
	FirstLevels *layer = [FirstLevels node];
...
//	NSLog(@"Attempting to update timer.");
...
//		NSLog(@"Trial mode detected. Trying to set trialLabel.");
...
//	NSLog(@"Trial label set.");
	if(changingLevel) {
		[SingletonGameState sharedGameStateInstance].time += 30;
		changingLevel = FALSE;
	}
...
//	NSLog(@"Attempting to add one to current level.");
	...
	if( (self=[super init] )) {
		...
		Flower* goodFlower = [Flower spriteWithFile: @"flower2.png"];
	...
	for(Flower *flower in [flowers reverseObjectEnumerator]) {
        if (CGRectContainsPoint([flower boundingBox], convertedPoint)) {
			if(flower.isGood == YES) {
				...
				if([SingletonGameState sharedGameStateInstance].level >= 5) {
					[[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:1 scene:[SecondLevels node]]];	
				}
				else {
					[[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:1 scene:[FirstLevels node]]];
