...
#import "GameOver.h"
#import "FifthLevels.h"
#import "Flower.h"
#import "SingletonGameState.h"
#import "FourthLevels.h"

@implementation FourthLevels
+(id) scene {
	// 'scene' is an autorelease object.
	CCScene *scene = [CCScene node];
	
	// 'layer' is an autorelease object.
	FourthLevels *layer = [FourthLevels node];
...
-(void)updateTime:(id)sender {
	...
	if(changingLevel) {
			[SingletonGameState sharedGameStateInstance].time += 20;
			changingLevel = FALSE;
		}
...
//	NSLog(@"Current level: %i", [SingletonGameState sharedGameStateInstance].level);
	...
	if( (self=[super init] )) {
		CCSprite *bg1 = [CCSprite spriteWithFile:@"bg51.png"];
		[bg1 setPosition:CGPointMake(240, 240)];
		[self addChild:bg1];
		CCSprite *bg2 = [CCSprite spriteWithFile:@"bg52.png"];
		[bg2 setPosition:CGPointMake(240, 60)];
		[self addChild:bg2];
		CGSize winSize = [[CCDirector sharedDirector] winSize];
		int winWidth = winSize.width - 25;
		int winHeight = winSize.height - 25;
		int flowersToDraw = ([SingletonGameState sharedGameStateInstance].level-14)*5+15;
		// create badFlowers
		for(int x=0;x<flowersToDraw;x++) {
			int badX = (arc4random()%winWidth) +15;
			int badY = (arc4random()%winHeight) +15;
			Flower* badFlower = [Flower spriteWithFile: @"badFlower5.png"];
		...
		Flower* goodFlower = [Flower spriteWithFile: @"flower5.png"];
...
//				NSLog(@"Good flower touched.");
				if([SingletonGameState sharedGameStateInstance].level >= 20) {
					[[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:1 scene:[FifthLevels node]]];	
				}
				else {
					[[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:1 scene:[FourthLevels node]]];
				}
				changingLevel = TRUE;
			}
		}
